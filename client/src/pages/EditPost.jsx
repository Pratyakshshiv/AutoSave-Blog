...
export default function EditPost() {
  ...
  const [id, setId] = useState(idData?.id);
  ...
  const clearPhoto = (e) => {
    ...
    PhotoData.set("id", id);
    axios.put("/delete/cover", { id, cover });
    setCover(null);
    setFiles(null);
  ...
  useEffect(() => {
    // To prevent Trigger due to first load of data
    if (
  ...
  // If clicked on Save Post then Redirect to HomePage
  if (redirect) {
    return <Navigate to={"/"} />;
  }
  return (
    <form onSubmit={SavePost}>
      ...
      <button style={{ marginTop: "5px" }}>Save post</button>
      <ToastContainer />
    </form>
  );
}
